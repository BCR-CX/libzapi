...
MODULE = "libzapi.infrastructure.api_clients.conversations.app_key_api_client"

# ── Hypothesis ──────────────────────────────────────────────────────────

strategy = builds(AppKey, id=just("key-1"))


@given(strategy)
def test_logical_key(model):
    assert model.logical_key.as_str() == "app_key:key-1"
...
def test_list_all_calls_correct_path(mocker):
    ...
    client = AppKeyApiClient(https)
    list(client.list_all("app-1"))

    https.get_raw.assert_called_with("https://example.zendesk.com/sc/v2/apps/app-1/keys?page[size]=100")
...
def test_get_calls_correct_path(mocker):
    ...
    client = AppKeyApiClient(https)
    client.get("app-1", "key-1")

    https.get.assert_called_with("/v2/apps/app-1/keys/key-1")
...
def test_create_calls_correct_path(mocker):
    ...
    client = AppKeyApiClient(https)
    client.create("app-1", "My Key")

    https.post.assert_called_with("/v2/apps/app-1/keys", {"displayName": "My Key"})
...
def test_delete_calls_correct_path(mocker):
    ...
    client = AppKeyApiClient(https)
    client.delete("app-1", "key-1")

    https.delete.assert_called_with("/v2/apps/app-1/keys/key-1")
